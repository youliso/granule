  ...
  if (typeof obj === 'object') {
    for (let key in obj) {
      if (typeof obj[key] === 'object') {
        obj[key] = newProxyArray(
          obj[key],
          callback,
          `${sKey ? `${sKey}.` : ''}${key as unknown as string}`
        );
      }
    }
  }
    ...
    set: function (target, key, value, receiver) {
      ...
      if (!(Array.isArray(target) && key === 'length')) {
        callback(cbType, {
          ...
          key,
          value,
          aKey: sKey ? `${sKey}.${key as unknown as string}` : ''
        });
      }
...
export function useProxy<T>(value: T): [ProxyValue<T>, (...arg: any) => T] {
  ...
  let data = newProxyArray({ value }, (...arg: any) => funks.forEach((e) => e(...arg)));
  return [data, func];
}
...
export function useElement<T>(
  ...
  const data = newProxyArray({ value }, (type: any, ob: any) => {
    ...
    els[key] && els[key].forEach((el) => (el.textContent = JSON.parse(JSON.stringify(ob.value))));
  });
